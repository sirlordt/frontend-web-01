...
function signupInstagram( payload: any ): any {

  return async ( dispatch: any ) => {

    const result = await systemBackendClient.callUserSignupInstagram( payload.signupData );

    if ( result instanceof Error ) {

      dispatch(
        {
          type: _USER_SIGNUP_INSTAGRAM_FAILED,
          payload: {
            response: {
              StatusCode: 400,
      ...
      if ( result.output.body.IsError === false &&
        ...
        dispatch(
          {
            type: _USER_SIGNUP_INSTAGRAM_SUCCESS,
            payload: {
