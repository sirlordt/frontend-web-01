...
function signupFacebook( payload: any ): any {

  return async ( dispatch: any ) => {

    const result = await systemBackendClient.callUserSignupFacebook( payload.signupData );

    if ( result instanceof Error ) {

      dispatch(
        {
          type: _USER_SIGNUP_FACEBOOK_FAILED,
          payload: {
            response: {
              StatusCode: 400,
      ...
      if ( result.output.body.IsError === false &&
        ...
        dispatch(
          {
            type: _USER_SIGNUP_FACEBOOK_SUCCESS,
            payload: {
